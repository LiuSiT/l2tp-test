    ...
    def __init__(self, label, origin=None):
        if isinstance(label, DNSLabel):
            ...
            if not label or label in (b'.', '.'):
                ...
                self.label = tuple(label.encode(
                    "idna").rstrip(b".").split(b"."))
    ...
    def pack(self, fmt, *args):
        for i, d in zip(fmt, args):
            if i == 'B':
                ...
                if len(d) > 255:
                    raise BufferError(
                        "Error packing struct '%s' <%s>: too long string %d" % (fmt, data, len(d)))
                ...
                raise BufferError(
                    "Error packing struct '%s' <%s>: unknown fmt '%s'" % (fmt, data, i))
        ...
        for i in fmt:
            if i == 'B':
                ...
                raise BufferError("Error unpacking struct '%s' <%s>" % (
                    fmt, binascii.hexlify(self.data).decode()))
        return data
...
class DNSRecord(object):
    ...
    def reply(self, ra=1, aa=1):
        header = DNSHeader(self.header.id, self.header.bitmap,
                           0, 0, 0, 0, qr=1, ra=ra, aa=aa)
...
def make_time(d): return int(time.mktime(
    time.strptime(d+'GMT', "%Y%m%d%H%M%S%Z")))


def make_hex(d): return binascii.unhexlify(d.encode('ascii'))
...
class Struct(object):
    ...
    def unpack(cls, buffer):
        try:
            ...
            raise DNSError("Error unpacking %s [offset=%d]: %s" % (
                cls.__name__, buffer.offset, e))
...
class RR(object):
    ...
    def unpack(cls, buffer):
        try:
            ...
            raise DNSError(
                "Error unpacking RR [offset=%d]: %s" % (buffer.offset, e))
...
class RRSIG(Struct):
    STRUCT = "HBBIIIHlD"
    ATTRS = ('covered', 'algorithm', 'labels', 'orig_ttl',
