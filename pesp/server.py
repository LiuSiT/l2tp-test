...
class IKEv1Session:
    ...
    def response(self, exchange, payloads, message_id=0, *, crypto=None, hashmsg=None):
        if hashmsg:
            ...
            buf = (b'' if hashmsg is True else hashmsg) + \
                message.Message.encode_payloads(payloads)
            hash_r = self.crypto.prf.prf(
                self.skeyid_a, message_id.to_bytes(4, 'big') + buf)
    ...
    def verify_hash(self, request):
        ...
        hash_i = self.crypto.prf.prf(self.skeyid_a, request.message_id.to_bytes(
            4, 'big') + message.Message.encode_payloads(request.payloads))
    ...
    def xauth_init(self):
        ...
        response_payloads = [message.PayloadCP_1(
            enums.CFGType.CFG_REQUEST, attrs)]
    ...
    def process(self, request, stream, remote_id, reply):
        ...
        if request.exchange == enums.Exchange.IDENTITY_1 and request.get_payload(enums.Payload.SA_1):
            ...
            self.peer_public_key = request.get_payload(
                enums.Payload.KE_1).ke_data
            self.my_public_key, self.shared_secret = crypto.DiffieHellman(
                self.transform[enums.TransformAttr.DH], self.peer_public_key)
            ...
            cipher = crypto.Cipher(
                self.transform[enums.TransformAttr.ENCR], self.transform[enums.TransformAttr.KEY_LENGTH])
            prf = crypto.Prf(self.transform[enums.TransformAttr.HASH])
            self.skeyid = prf.prf(self.args.passwd.encode(),
                                  self.peer_nonce+self.my_nonce)
            self.skeyid_d = prf.prf(
                self.skeyid, self.shared_secret+self.peer_spi+self.my_spi+bytes([0]))
            self.skeyid_a = prf.prf(
                self.skeyid, self.skeyid_d+self.shared_secret+self.peer_spi+self.my_spi+bytes([1]))
            self.skeyid_e = prf.prf(
                self.skeyid, self.skeyid_a+self.shared_secret+self.peer_spi+self.my_spi+bytes([2]))
            iv = prf.hasher(self.peer_public_key +
                            self.my_public_key).digest()[:cipher.block_size]
            self.crypto = crypto.Crypto(
                cipher, self.skeyid_e[:cipher.key_size], prf=prf, iv=iv)
            reply(self.response(enums.Exchange.IDENTITY_1, response_payloads))
            ...
            prf = self.crypto.prf
            hash_i = prf.prf(self.skeyid, self.peer_public_key+self.my_public_key +
                             self.peer_spi+self.my_spi+self.sa_bytes+payload_id.to_bytes())
            assert hash_i == request.get_payload(
                enums.Payload.HASH_1).data, 'Authentication Failed'
            response_payload_id = message.PayloadID_1(
                enums.IDType.ID_FQDN, f'{__title__}-{__version__}'.encode())
            hash_r = prf.prf(self.skeyid, self.my_public_key+self.peer_public_key +
                             self.my_spi+self.peer_spi+self.sa_bytes+response_payload_id.to_bytes())
            response_payloads = [response_payload_id,
                                 message.PayloadHASH_1(hash_r)]
            reply(self.response(enums.Exchange.IDENTITY_1,
                                response_payloads, crypto=self.crypto))
            ...
            if enums.CPAttrType.XAUTH_USER_NAME in payload_cp.attrs:
                ...
                response_payloads = [message.PayloadCP_1(
                    enums.CFGType.CFG_SET, {enums.CPAttrType.XAUTH_STATUS: 1})]
                ...
                response_payloads = [message.PayloadCP_1(
                    enums.CFGType.CFG_REPLY, attrs, identifier=payload_cp.identifier)]
                self.state = State.CONF_SENT
            elif payload_cp.cftype == enums.CFGType.CFG_ACK:
                return
            else:
                raise Exception('Unknown CP Exchange')
            reply(self.response(enums.Exchange.TRANSACTION_1, response_payloads,
                                request.message_id, crypto=self.crypto, hashmsg=True))
            ...
            chosen_proposal = request.get_payload(
                enums.Payload.SA_1).proposals[0]
            del chosen_proposal.transforms[1:]
            peer_spi = chosen_proposal.spi
            chosen_proposal.spi = my_spi = os.urandom(4)
            reply(self.response(enums.Exchange.QUICK_1, request.payloads,
                                request.message_id, crypto=self.crypto, hashmsg=peer_nonce))

            transform = chosen_proposal.transforms[0].values
            cipher = crypto.Cipher(
                chosen_proposal.transforms[0].id, transform[enums.ESPAttr.KEY_LENGTH])
            integ = crypto.Integrity(transform[enums.ESPAttr.AUTH])
            keymat_fmt = struct.Struct(
                '>{0}s{1}s'.format(cipher.key_size, integ.key_size))
            keymat = self.crypto.prf.prfplus(self.skeyid_d, bytes(
                [chosen_proposal.protocol])+my_spi+peer_nonce+my_nonce, False)
            sk_ei, sk_ai = keymat_fmt.unpack(
                bytes(next(keymat) for _ in range(keymat_fmt.size)))
            keymat = self.crypto.prf.prfplus(self.skeyid_d, bytes(
                [chosen_proposal.protocol])+peer_spi+peer_nonce+my_nonce, False)
            sk_er, sk_ar = keymat_fmt.unpack(
                bytes(next(keymat) for _ in range(keymat_fmt.size)))
            ...
            if not request.payloads:
                ...
                for spi in delete_payload.spis:
                    child_sa = next(
                        (x for x in self.child_sa if x.spi_out == spi), None)
                    if child_sa:
                        self.child_sa.remove(child_sa)
                        self.sessions.pop(child_sa.spi_in)
                        spis.append(child_sa.spi_in)
                response_payloads.append(message.PayloadDELETE_1(
                    delete_payload.doi, delete_payload.protocol, spis))
                message_id = request.message_id
            ...
            reply(self.response(enums.Exchange.INFORMATIONAL_1,
                                response_payloads, message_id, crypto=self.crypto, hashmsg=True))
...
class IKEv2Session:
    ...
    def create_key(self, ike_proposal, shared_secret, old_sk_d=None):
        ...
        integ = crypto.Integrity(
            ike_proposal.get_transform(enums.Transform.INTEG).id)
        ...
        if not old_sk_d:
            ...
            skeyseed = prf.prf(old_sk_d, shared_secret +
                               self.peer_nonce+self.my_nonce)
        keymat_fmt = struct.Struct('>{0}s{1}s{1}s{2}s{2}s{0}s{0}s'.format(
            prf.key_size, integ.key_size, cipher.key_size))
        keymat = prf.prfplus(skeyseed, self.peer_nonce +
    ...
    def create_child_key(self, child_proposal, nonce_i, nonce_r):
        integ = crypto.Integrity(
            child_proposal.get_transform(enums.Transform.INTEG).id)
        ...
        keymat_fmt = struct.Struct(
            '>{0}s{1}s{0}s{1}s'.format(cipher.key_size, integ.key_size))
        keymat = self.my_crypto.prf.prfplus(self.sk_d, nonce_i+nonce_r)
        sk_ei, sk_ai, sk_er, sk_ar = keymat_fmt.unpack(
            bytes(next(keymat) for _ in range(keymat_fmt.size)))
        crypto_in = crypto.Crypto(cipher, sk_ei, integ, sk_ai)
        crypto_out = crypto.Crypto(cipher, sk_er, integ, sk_ar)
        child_sa = ChildSa(os.urandom(4), child_proposal.spi,
    ...
    def process(self, request, stream, remote_id, reply):
        ...
        if request.exchange == enums.Exchange.IKE_SA_INIT:
            ...
            if payload_ke.dh_group != prefered_dh or payload_ke.ke_data[0] == 0:
                ...
                return
            public_key, shared_secret = crypto.DiffieHellman(
                payload_ke.dh_group, payload_ke.ke_data)
            ...
            response_payloads = [message.PayloadSA([chosen_proposal]),
                                 ...
                                 message.PayloadKE(
                payload_ke.dh_group, public_key),
                message.PayloadNOTIFY(
                0, enums.Notify.NAT_DETECTION_DESTINATION_IP, b'', os.urandom(20)),
            ...
            response_payload_idr = message.PayloadIDr(
                enums.IDType.ID_FQDN, f'{__title__}-{__version__}'.encode())
            auth_data = self.auth_data(
                self.response_data, self.peer_nonce, response_payload_idr, self.my_crypto.sk_p)
            ...
            if request.get_payload(enums.Payload.CP):
                attrs = {enums.CPAttrType.INTERNAL_IP4_ADDRESS: ipaddress.ip_address('1.0.0.1').packed,
                         enums.CPAttrType.INTERNAL_IP4_DNS: ipaddress.ip_address(self.args.dns).packed, }
                response_payloads.append(message.PayloadCP(
                    enums.CFGType.CFG_REPLY, attrs))
            ...
            if not request.payloads:
                ...
                response_payloads.append(
                    message.PayloadDELETE(delete_payload.protocol, spis))
            else:
                raise Exception(f'unhandled informational {request!r}')
            reply(self.response(enums.Exchange.INFORMATIONAL,
                                response_payloads, crypto=self.my_crypto))
            ...
            if chosen_proposal.protocol != enums.Protocol.IKE:
                ...
                if not payload_notify:
                    raise Exception(
                        f'unhandled protocol {chosen_proposal.protocol} {request!r}')
                old_child_sa = next(
                ...
                child_sa = self.create_child_key(
                    chosen_proposal, peer_nonce, my_nonce)
                ...
                child = IKEv2Session(
                    self.args, self.sessions, chosen_proposal.spi)
                child.state = State.ESTABLISHED
                child.peer_nonce = request.get_payload(
                    enums.Payload.NONCE).nonce
            ...
            reply(self.response(enums.Exchange.CREATE_CHILD_SA,
                                response_payloads, crypto=self.my_crypto))
        else:
            raise Exception(f'unhandled request {request!r}')
...
class IKE_500(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr, *, response_header=b''):
        ...
        if request.exchange == enums.Exchange.IKE_SA_INIT:
            ...
            return
        session.process(request, stream, addr[:2], lambda response: self.transport.sendto(
            response_header+response, addr))
...
class SPE_4500(IKE_500):
    ...
    def datagram_received(self, data, addr):
        ...
        if spi == b'\xff':
            ...
            IKE_500.datagram_received(
                self, data[4:], addr, response_header=IKE_HEADER)
...
def main():
    parser = argparse.ArgumentParser(
        description=__description__, epilog=f'Online help: <{__url__}>')
    parser.add_argument('-r', dest='rserver', default=[], action='append',
                        type=pproxy.Connection, help='tcp remote server uri (default: direct)')
    parser.add_argument('-ur', dest='urserver', default=[], action='append',
                        type=pproxy.Connection, help='udp remote server uri (default: direct)')
    parser.add_argument('-s', dest='salgorithm', default='fa', choices=('fa',
                                                                        'rr', 'rc', 'lc'), help='scheduling algorithm (default: first_available)')
    parser.add_argument('-p', dest='passwd', default='test',
                        help='password (default: test)')
    parser.add_argument('-dns', dest='dns', default='1.1.1.1',
                        help='dns server (default: 1.1.1.1)')
    parser.add_argument('-nc', dest='nocache', default=None,
                        action='store_true', help='do not cache dns (default: off)')
    ...
    sessions = {}
    transport1, _ = loop.run_until_complete(loop.create_datagram_endpoint(
        lambda: IKE_500(args, sessions), ('0.0.0.0', 500)))
    transport2, _ = loop.run_until_complete(loop.create_datagram_endpoint(
        lambda: SPE_4500(args, sessions), ('0.0.0.0', 4500)))
