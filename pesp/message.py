            ...
            if protocol == enums.Protocol.ESP:
                ...
                for attr_type in values:
                    if attr_type in enums.ESPTable_1:
                        values[attr_type] = enums.ESPTable_1[attr_type](
                            values[attr_type])
                ...
                for attr_type in values:
                    if attr_type in enums.TransformTable_1:
                        values[attr_type] = enums.TransformTable_1[attr_type](
                            values[attr_type])
                transforms.append(Transform_1(
                    tnum, enums.Protocol(id), values))
        return Proposal_1(num, protocol, spi, transforms)

    def to_bytes(self):
        data = bytearray()
        data.extend(struct.pack('>BBBB', self.num, self.protocol,
...
class PayloadSA_1(Payload):
    ...
    def to_bytes(self):
        ...
        for idx, proposal in enumerate(self.proposals):
            ...
            data.extend(struct.pack('>BxH', 0 if idx == len(
                self.proposals)-1 else 2, len(proposal_data)+4))
...
class PayloadNONCE_1(Payload):
    def __init__(self, nonce=None, critical=False):
        ...
        self.nonce = os.urandom(random.randrange(
            16, 256)) if nonce is None else nonce
...
class PayloadNOTIFY_1(Payload):
    ...
    def parse_data(self, stream, length):
        self.doi, protocol, spi_size, notify = struct.unpack(
            '>IBBH', stream.read(8))
        self.protocol = enums.Protocol(protocol)
        self.notify = enums.Notify(notify)
    ...
    def to_bytes(self):
        data = bytearray(struct.pack('>IBBH', self.doi,
                                     self.protocol, len(self.spi), self.notify))
        data.extend(self.spi)
...
class PayloadDELETE_1(Payload):
    ...
    def parse_data(self, stream, length):
        self.doi, protocol, spi_size, num_spis = struct.unpack(
            '>IBBH', stream.read(8))
        self.protocol = enums.Protocol(protocol)
    ...
    def to_bytes(self):
        data = bytearray()
        data.extend(struct.pack('>IBBH', self.doi, self.protocol, len(
            self.spis[0]) if self.spis else 0, len(self.spis)))
...
class Proposal:
    ...
    def parse(cls, stream):
        ...
        while more:
            ...
            transforms.append(Transform(enums.Transform(
                type), enums.TransformTable[type](id), keylen))
    ...
    def to_bytes(self):
        ...
        for idx, transform in enumerate(self.transforms):
            transform_data = struct.pack(
                '>HH', 0x800e, transform.keylen) if transform.keylen else b''
...
class PayloadAUTH(Payload):
    ...
    def parse_data(self, stream, length):
        self.method = enums.AuthMethod(
            struct.unpack('>B3x', stream.read(4))[0])
...
class PayloadNOTIFY(Payload):
    ...
    def to_bytes(self):
        data = bytearray(struct.pack(
            '>BBH', self.protocol, len(self.spi), self.notify))
        data.extend(self.spi)
        data.extend(self.data)
        return data
...
class PayloadDELETE(Payload):
    ...
    def to_bytes(self):
        data = bytearray()
        data.extend(struct.pack('>BBH', self.protocol, len(
            self.spis[0]) if self.spis else 0, len(self.spis)))
        for spi in self.spis:
            data.extend(spi)
        return data
...
class TrafficSelector:
    ...
    @classmethod
    def parse(cls, stream):
        ts_type, ip_proto, length, start_port, end_port = struct.unpack(
            '>BBHHH', stream.read(8))
        addr_len = (length - 8) // 2
...
class PayloadTSi(Payload):
    ...
    def parse_data(self, stream, length):
        ...
        self.traffic_selectors = [
            TrafficSelector.parse(stream) for i in range(n_ts)]

    def to_bytes(self):
        data = bytearray(struct.pack(
            '>BBH', len(self.traffic_selectors), 0, 0))
...
class Message:
    ...
    def parse_payloads(self, stream, *, crypto=None):
        if self.flag & enums.MsgFlag.Encryption:
            stream = io.BytesIO(crypto.decrypt_1(
                stream.read(), self.message_id))
        ...
        while next_payload:
            ...
            payload = PayloadClass.get(payload_id, Payload).parse(
                payload_id, critical, stream, length-4)
    ...
    def encode_payloads(cls, payloads):
        ...
        for idx, payload in enumerate(payloads):
            ...
            if idx < len(payloads) - 1:
                ...
                next_payload = enums.Payload.NONE
            data.extend(struct.pack('>BBH', next_payload,
                                    0x80 if payload.critical else 0x00, len(payload_data) + 4))
    ...
    def to_bytes(self, *, crypto=None):
        ...
        if crypto and self.version == 0x10:
            ...
            data = bytearray(struct.pack('>BxH', first_payload,
                                         len(payload_data) + 4) + payload_data)
